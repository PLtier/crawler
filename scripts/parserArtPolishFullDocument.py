...
def chunk_document(
    ...
    with open(input_file, 'r', encoding='utf-8') as f:
        ...
        # PRE-PROCESSING: REMOVE "Treść obwieszczenia"
        ...
        #pre-pass: identify elements belong to xtext (these are the main elements of interest - Arts and subelements.)
        fused_headers_set = set()
        for xnode in soup.select('[data-template="xText"]'):
            parent = xnode.parent
            if parent:
                for sibling in parent.previous_siblings:
                    if getattr(sibling, 'name', None) == 'h3':
                        h3_text = ' '.join(sibling.get_text(separator=' ').split())
                        fused_headers_set.add(sibling)
                        if h3_text == '1.':
                            ggp = parent.parent.parent
                            if ggp:
                                for p_sibling in ggp.previous_siblings:
                                    if getattr(p_sibling, 'name', None) == 'h3':
                                        fused_headers_set.add(p_sibling)
                                        break
                        break

        #first pass: dom walker (catch everything)
        def get_all_text_blocks(element):
            blocks = []
            if not isinstance(element, Tag):
                return blocks
                
            if element.name == 'div' and element.get('data-template') == 'xText':
                return [element]
                
            if element.name in ['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'table', 'ul', 'ol']:
                # ONLY yield it if it's not going to be fused by the old logic
                if element not in fused_headers_set:
                    return [element]
                return [] 
                
            for child in element.children:
                blocks.extend(get_all_text_blocks(child))
                
            return blocks

        target_nodes = get_all_text_blocks(soup.body) if soup.body else []
        ...
        #second pass: chunking and reference handling
        seq = 1
        ...
        for node in target_nodes:
            ...
            is_xtext = node.name == 'div' and node.get('data-template') == 'xText'
            chunk_text = ' '.join(node.get_text(separator=' ').split())
            ...
            if current_id:
                ...
                for match in ref_pattern.finditer(regex_search_text):
                    if not match.group(0).strip(): continue
                    
                    pre_text = regex_search_text[max(0, match.start()-60):match.start()]
                    post_text = regex_search_text[match.end():match.end()+120]
